...
BEGIN
    ...
    -- Calculando o 5º dia útil
    SET quintoDiaUtil = primeiroDiaUtil;
    SET @contador_diasUteis = 0;
    
    -- Enquanto contador_diasUteis for < 5, continua rodando o while
    WHILE @contador_diasUteis < 5 DO
        -- Adicionando um dia na variável quintoDiaUtil
        SET quintoDiaUtil = DATE_ADD(quintoDiaUtil, INTERVAL 1 DAY);
        
        -- Adicionando um IF para não incluir Sábados (1) e Domingos (7)
        IF DAYOFWEEK(quintoDiaUtil) NOT IN (1, 7) THEN
            -- Adicionando +1 na variável contador
            SET @contador_diasUteis = @contador_diasUteis + 1;
        END IF;
    END WHILE;
    
    -- Armazenando o resultado para utilizar fora do PROCEDURE
...
BEGIN
    ...
    -- Calculando o quinto dia útil do mês
    CALL CalcularQuintoDiaUtil(ano_param, mes_param, quintoDiaUtil);

    -- Excluindo registros existentes para o ano e mês de referência
...
-- Setando o quinto dia útil como nulo      
SET @resultado_QuintoDiaUtil = null;

-- Chamando o procedure, passando mês e ano e recebendo o resultado
-- O valor da variavel 'resultado' é atribuido a '@resultado_quintoDiaUtil', quando o call é chamado
...
-- Chama a procedure para gerar os registros de pagamento
CALL GerarRegistrosPagamento(@ano_referencia, @mes_referencia);

-- Seleciona os registros gerados na tabela FOLHA_PAGAMENTO
